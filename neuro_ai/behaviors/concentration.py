import numpy as np
from pre_processing.pre_procesing import ica, setup_mne_data
...
class Concentration:
    ...
    def pre_process_data(self, raw):
        mne_raw = setup_mne_data(raw)
        mne_raw.filter(4, 40, method="iir")
        data = ica(mne_raw)
        return data

    def concentration_level(self, mne_raw) -> int:
        ...
        concentration_thresholds = self.concentration_levels_threshold.values()
